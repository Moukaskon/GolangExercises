...
func main() {
	...
	fmt.Println("Calculator Server running on port 12345...")
	...
	for {
		...
		go handleCalculatorClient(conn) // ✅ each client handled in goroutine
	}
}

func handleCalculatorClient(conn net.Conn) {
	defer conn.Close()
